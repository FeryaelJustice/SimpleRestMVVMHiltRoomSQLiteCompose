...
class UserRepositoryTest {
    ...
    private val restDataSource = Retrofit.Builder().baseUrl(mockWebServer.url("/")).client(
        OkHttpClient.Builder().build()
    ).addConverterFactory(GsonConverterFactory.create()).build().create(RestDataSource::class.java)
    ...
    @Test
    fun `Users are deleted correctly`() {
        runBlocking {
    ...
    @Test
    fun `Users are fetched correctly`() {
        runBlocking {
